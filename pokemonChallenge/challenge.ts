...
//Function que procura um pokemon por nome:
 ...
 const searchPokemon = (pokemons: Pokemons, name: string): Pokemon | undefined=>
...
const filterPokemons = (pokemons: Pokemons, type: string): Pokemon[] | undefined=>
  pokemons.filter((pokemon) => pokemon.type.includes(type));

const sameTypeOfPokemons: Pokemon[] | undefined  = filterPokemons(pokemons, "bug");
...
const pokemonsInPTBR = pokemons.map((pokemon: Pokemon): Pokemon => {
  const changeTypesToPtBR = pokemon.type.map((type: string): string => pokemonTypesInPtBR[type]);
